                        ...
                        if target_tag in ["LROC", "TROC"] and not df_visits.empty:
                            ...
                            if not clinic_visits.empty:
                                with st.container(border=True):
                                    ...
                                    with c_v1:
                                        fig_ov = px.bar(latest_v_df.sort_values('Total Visits', ascending=True), x='Total Visits', y='Name', orientation='h', text_auto=True, color='Total Visits', color_continuous_scale='Blues', title=f"Total Office Visits (YTD {latest_v_date.strftime('%b %Y')})")
                                        st.plotly_chart(fig_ov, use_container_width=True)
                                    with c_v2:
                                        fig_np = px.bar(latest_v_df.sort_values('New Patients', ascending=True), x='New Patients', y='Name', orientation='h', text_auto=True, color='New Patients', color_continuous_scale='Greens', title=f"New Patients (YTD {latest_v_date.strftime('%b %Y')})")
