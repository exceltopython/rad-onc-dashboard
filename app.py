...
"""
    ...
    def safe_dedup_and_format(df_list, subset_cols):
        ...
        if 'Month_Clean' in df.columns:
            # Force all dates to the 1st of the month to ensure perfect merging
            df['Month_Clean'] = df['Month_Clean'].apply(standardize_date)
            df = df.dropna(subset=['Month_Clean'])
            # Sort fix: check for column existence to prevent KeyError
            sort_order = ['Month_Clean']
            if 'Total RVUs' in df.columns: sort_order.append('Total RVUs')
            df = df.sort_values(sort_order, ascending=[False, False])
        
        # Only deduplicate columns that actually exist in the current dataframe
                ...
                if df_clinic_26.empty:
                    ...
                    with col_main_26:
                        ...
                        if df_view_26.empty and clinic_filter_26 not in ["TriStar", "Ascension"]:
                            ...
                            # --- REPLACEMENT BLOCK (FIXES DUPLICATE COLUMN ERROR) ---
                            with st.container(border=True):
                                st.markdown("##### 📅 Historical Data Summary")
                                df_hist_source = get_historical_df()
                                
                                # Filter historical IDs based on your current view
                                if clinic_filter_26 == "TriStar": df_h_view = df_hist_source[df_hist_source['ID'].isin(TRISTAR_IDS)]
                                elif clinic_filter_26 == "Ascension": df_h_view = df_hist_source[df_hist_source['ID'].isin(ASCENSION_IDS)]
                                elif clinic_filter_26 == "All": df_h_view = df_hist_source.copy()
                                else: df_h_view = pd.DataFrame()
                                
                                if not df_h_view.empty:
                                    # Step 1: Group by Year to merge dictionary data
                                    hist_agg = df_h_view.groupby('Year')[['Total RVUs']].sum().reset_index()
                                    
                                    # Step 2: Add the calculated YTD from your Excel files for 2026
                                    if not df_clinic.empty:
                                        # We use a filter to get ONLY the 2026 sum from the processed files
                                        current_26_val = df_clinic[df_clinic['Month_Clean'].dt.year == 2026]['Total RVUs'].sum()
                                        
                                        if 2026 in hist_agg['Year'].values:
                                            hist_agg.loc[hist_agg['Year'] == 2026, 'Total RVUs'] = current_26_val
                                        else:
                                            hist_agg = pd.concat([hist_agg, pd.DataFrame({"Year": [2026], "Total RVUs": [current_26_val]})])
                            
                                    hist_agg['Year'] = hist_agg['Year'].astype(str)
                                    
                                    # THE FIX: Force unique Year index before Transposing
                                    # This collapses any 2025 data coming from both the dictionary AND the files
                                    hist_table_final = hist_agg.groupby('Year').sum().T
                                    st.dataframe(hist_table_final.style.format("{:,.0f}"), use_container_width=True)
                            
                            # --- RE-ADDING MONTHLY & QUARTERLY TABLES ---
